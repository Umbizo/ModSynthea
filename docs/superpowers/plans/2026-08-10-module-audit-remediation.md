...
**Post-upgrade, fix round 1 (Task 0.1, 2026-08-10):** `python3 tools_lint_module_locks.py` → **CRITICAL=67 WARNING=0**. `emergency` and `home` encounter classes are now strict-classed (same closed-before-blocked rule as ambulatory, not the permissive inpatient/snf/hospice one) — this surfaces 3 additional stock-module locks: `sepsis.json:Sepsis_ED_Encounter`, `gallstones.json:Cholecystitis_Encounter`, `injuries.json:ED_Visit_For_Spinal_Injury`. These are expected and out of scope (stock modules, triaged later, not edited); no other deltas versus the CRITICAL=62 baseline above.
...
- [ ] `python3 tools_lint_module_locks.py` → 0 CRITICAL outside stock modules. `./tools_smoke_gen.sh 1000` → no exceptions; `python3 tools_smoke_check.py --code 399068003 --sex M --eligible-min-age 55 --max-share 0.18 --mean-encounters-min 30 --top-share-max 0.05 --top-share-universe docs/superpowers/plans/2026-08-10-fork-condition-codes.txt` → PASS. Commit the recorded gate output: `chore(audit): phase 1 gate - encounter starvation eliminated`.
  - Adjudicated 2026-08-10: the exclude-list approach was a dead end (stock SDOH/administrative codes are a systemic family at full-lifetime export scale, not a short fixed list — chasing the leader with more excludes never converged). Replaced with `--top-share-universe`: the monoculture check now only considers codes from the branch's own touched fork modules as candidates for the "top" seat (denominator unchanged, still all condition rows), which is what the check exists to catch (a single fork-disease dominating, per the HOHF I50.83=98% incident). Universe file generated from `git diff --name-only feffd500..HEAD -- src/main/resources/modules/`, 46 ConditionOnset codes across the 14 touched modules. Re-run result: top fork code is PKD's `59621000` "Essential hypertension" at 0.0025 share (821/333348), pneumonia `233604007` a close second at 0.0024 — both far under 0.05. See `docs/superpowers/plans/2026-08-10-phase1-gate.txt` section 9 for the full re-run output.
...
- [ ] **7.2 Generation gate:** `./tools_smoke_gen.sh 1000` → no exceptions. Assertions, all must PASS: `--mean-encounters-min 30 --top-share-max 0.05 --top-share-universe docs/superpowers/plans/2026-08-10-fork-condition-codes.txt` (starvation/monoculture, restricted to the branch's own fork-module disease codes — regenerate the universe file from `git diff --name-only feffd500..HEAD -- src/main/resources/modules/` against the Phase 7 tip before this gate, since more modules will have been touched by then); prostate 399068003 M 55+ share 0.06–0.18; AD 26929004 65+ share ≤ 0.15 (the Phase 1 gate used an interim 0.20 ceiling pending coordination Tasks 4.2/5.2 on the stock-dementia double-diagnosis; this Phase 7 gate restores the intentional 0.15 ceiling — Tasks 4.2/5.2 must be complete first or this will legitimately fail); pneumonia share 0.10–0.45 with onset 5th percentile ≥ 1 yr; MM/MCL/CIDP/ovarian/bladder/pancreatic shares within their task bounds; every custom-module target drug from the audit (pegfilgrastim, daratumumab, pembrolizumab, anti-VEGF, IVIG) appears ≥ once in `medications.csv`. Save the full check output as `docs/superpowers/plans/2026-08-10-final-verification.txt`.
